...
local function parse_battery_info(content)
  ...
  for status, power, energy, charge in content:gmatch(
    '.*STATUS=(%a+).+VOLTAGE_NOW=(%d+).+CURRENT_NOW=(%d+).+CAPACITY=(%d+)') do
...
function battery.init(args)
  ...
  battery.image = wibox.widget {
    image = args.path_to_icons .. "battery-empty-symbolic.svg",
    widget = wibox.widget.imagebox,
  }
  battery.text = wibox.widget.textbox()
  battery.text:set_markup(string.format("<span color=%q><b>%s%%</b></span>", beautiful.bg_normal, 0))
  ...
  monitor(unpack(merge(get_batteries(), {function(content)
    ...
    battery.text:set_markup(string.format("<span color=%q><b>%s%%</b></span>",
    ...
    battery.image:set_image(args.path_to_icons .. batteryType .. ".svg")
